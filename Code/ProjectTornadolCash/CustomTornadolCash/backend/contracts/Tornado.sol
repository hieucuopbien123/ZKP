...
contract Tornado is ReentrancyGuard {
  ...
  // Các node ở từng level có giá trị default random chứ k lấy 0x00
  ...
  function deposit(uint256 _commitment) external payable nonReentrant{
    ...
    uint256 newRoot;
    uint256[10] memory hashPairings; // Chỉ dùng phát sự kiện, frontend bắt là có cây merklepath thời điểm này luôn
    uint8[10] memory hashDirections; // 0 là bên trái, 1 là bên phải
    ...
    // Nó xây lên root mới nhất từ commitment hiện tại + update node mới nhất của từng level + phát event chứa merklepath của node leaf mới
    // Điều hay là contract chỉ lưu node mới nhất của từng level chứ k lưu cả cây merkle tree
    for(uint8 i = 0; i < treeLevel; i++){
      ...
      currentHash = h;
      currentIdx = currentIdx / 2; // Ra index node cha ở level trên
    }
    ...
    // Sự kiện này phát ra root ở thời điểm người này deposit, kp là root mới nhất về sau
  }
  ...
  function withdraw(
    ...
    uint[2] calldata _pC, 
    uint[2] calldata input // 1 phần tử cuối là receipient thì lấy là msg.sender luôn
